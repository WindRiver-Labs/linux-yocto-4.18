...
struct zynqmp_r5_rproc_pdata {
	struct rproc *rproc;
	struct rproc_ops fw_ops;
	const struct rproc_ops *default_fw_ops;
			...
			const struct firmware *fw)
{
	...
	rsc = local->default_fw_ops->find_loaded_rsc_table(rproc, fw);
	if (!rsc) {
...
static int zynqmp_r5_remoteproc_probe(struct platform_device *pdev)
{
	...
	memcpy(&local->fw_ops, rproc->ops, sizeof(local->fw_ops));
	local->fw_ops.find_loaded_rsc_table = r5_rproc_find_rsc_table;
	local->default_fw_ops = rproc->ops;
	rproc->ops = &local->fw_ops;
