...
#define DRV_NAME "xlnx-formatter-pcm"
...
static int xlnx_formatter_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	return snd_pcm_lib_preallocate_pages_for_all(rtd->pcm,
			SNDRV_DMA_TYPE_DEV, snd_soc_rtdcom_lookup(rtd, DRV_NAME)->dev,
...
static struct snd_soc_component_driver xlnx_asoc_platform = {
	.name = DRV_NAME,
...
static int xlnx_formatter_pcm_probe(struct platform_device *pdev)
{
	...
	ret = devm_snd_soc_register_component(&pdev->dev, &xlnx_asoc_platform, NULL, 0);
	if (ret) {
